...
describe RomanNumeralsFileProcessor do
  ...
  describe '#download_from_s3' do
    ...
    context 'when the file is successfully downloaded' do
      specify { expect(subject.download_from_s3(file_name)).to eq(file) }
    end

    context 'when directory does not exist' do
      let(:directory_name) { 'bad directory name' }
      before { allow(directories).to receive(:get).with(directory_name) { nil } }
      specify { expect(subject.download_from_s3(file_name)).to eq(nil) }
    end
  ...
  describe '#upload_to_s3' do
    context 'when the file is correctly uploaded' do
      let(:generated_file_name) { "Anya/#{file_name}_converted_#{Time.now.strftime("%H:%M:%S-%m-%d-%Y")}" }
      let(:public) { false }
      ...
      before do
        ...
        subject.upload_to_s3(file_name, numbers)
      end
